/**
 * Skip Navigation Link
 *
 * Provides keyboard users a way to skip directly to main content,
 * bypassing navigation and header elements. This is a WCAG 2.4.1
 * Level A requirement.
 *
 * The link is visually hidden until focused, maintaining a clean
 * visual design while providing essential keyboard navigation.
 */
export function SkipLink() {
  return (
    <a
      href="#main-content"
      className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:bg-blue-600 focus:text-white focus:rounded-md focus:shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
    >
      Skip to main content
    </a>
  );
}
